...
/**
 * Tier-scoped player-detail page: BiS plan + per-floor pages stats
 * + materials need + savage drop counter, all in the context of one
 * specific tier.
 *
 * v2.0 separates a player's stable identity from their tier-specific
 * gear plan. Stable identity (name, jobs, gear-link, notes) lives
 * once on `/team/[id]`; the BiS plan (current/desired source per
 * slot) is tier-scoped because each tier has a different `max_ilv`
 * and the team replans on every rollover.
 *
 * The BiS table here writes through `saveBisChoice` against the
 * (player_id, tier_id, slot) composite key in `bis_choice` so edits
 * affect *this* tier's plan only — switching to a different tier's
 * detail page shows a fresh plan, untouched by what the team
 * filled out for the previous tier.
 *
 * The Plan tab on `/tiers/[id]` reads the same `bis_choice` rows
 * via `loadPlayerSnapshots`, so anything edited here updates the
 * recommendation engine for this tier on the next render — and only
 * for this tier.
 */
export default async function TierScopedPlayerDetailPage({
  params,
}: {
  ...
  const playerId = Number.parseInt(pid, 10);
  if (
  ...
  // Validate that the tier belongs to the active team and the
  // player exists. We cross-check both before doing any heavier
  // reads so 404s short-circuit fast.
  const team = await getCurrentTeam();
  const [tier, player] = await Promise.all([
    findTierById(team.id, tierId),
    findPlayer(playerId),
  ]);
  if (!tier || !player || player.teamId !== team.id) {
    notFound();
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-screen-xl flex-1 flex-col gap-6 px-6 py-8">
      ...
      >
        ...
        {tPlayers("detail.backToTier", { tierName: tier.name })}
      </Link>
